...
public class EquivalenceTesting {
	...
	public static void main(String[] args) {
		...
		try{
			...
			bw_comparison = new BufferedWriter(new FileWriter(new File(new File(ModelsFromRealSULs_utils.getInstance().getVersions()[0]).getParentFile(),"equivTesting_client.tab")));
			bw_comparison.write("Equivalent\tSUL\tPrevious\n");
			
			for (int i = 0; i < ModelsFromRealSULs_utils.getInstance().getVersions().length; i++) {
				for (int j = 0; j < i; j++) { if (i==j) continue;

				File file_i = new File(ModelsFromRealSULs_utils.getInstance().getVersions()[i]);
				...
				File file_j = new File(ModelsFromRealSULs_utils.getInstance().getVersions()[j]);
	...
	private static void generate_OTsFromWset() throws Exception {
		...
		for (int i = 0; i < ModelsFromRealSULs_utils.getInstance().getVersions().length; i++) {
			...
			File file = new File(ModelsFromRealSULs_utils.getInstance().getVersions()[i]);
			// create out_dir
...
//		bw_comparison = new BufferedWriter(new FileWriter(new File(new File(ModelsFromRealSULs_utils.getInstance().getVersions()[0]).getParentFile(),ModelsFromRealSULs_utils.getInstance().getTab_filename()),true));
