...
public class SimpleExample {
	...
	public static void main(String[] args) throws Exception {
		...
		String [] files = dir.list(new FilenameFilter() {
			@Override
			public boolean accept(File dir, String name) { return name.matches("prod[0-9_]+.txt");}
		});

		// logfile for each set of FSM 
		FileHandler fh = new FileHandler(dir.getName()+"/"+dir.getName()+".log");
		...
		for(boolean[] conf: configs){
			...
			{		
				...
				for (String file_s : files) {
					...
					scenario_name = dir.getName()+"_"+file_s.substring(0, file_s.length()-4);
					...
					// reuse suffixes and prefixes previously considered
					

					Set<Word<String>> initCes = new HashSet<Word<String>>();
					initCes.add(Word.epsilon());
					// reuse all inputs (prefixes) using the same alphabet
					if (okCe){
						for (Word<String> word : allPrefixes) {
							boolean inclOk = true;
