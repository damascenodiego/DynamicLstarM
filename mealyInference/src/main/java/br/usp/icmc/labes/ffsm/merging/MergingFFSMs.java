package br.usp.icmc.labes.ffsm.merging;
...
public class MergingFFSMs {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// load mealy machine
			CompactMealy<String, Word<String>> mealyIO = Utils.getInstance().loadMealyTrace(io);

			BufferedWriter bw = new BufferedWriter(new FileWriter(z3_file));
			
			int nBlocks = 3;
			bw.write("; set variables\n");
			...
			bw.write("\n");
			bw.write("; set constraints that each state should be in **AT MOST** in one block\n");
			for (Integer state : mealyIO.getStates()) {
				bw.write("(assert (and");
			...
			bw.write("\n");
			bw.write("; set constraints to simplify learning\n");
			for (Integer statei : mealyIO.getStates()) {
				for (Integer statej : mealyIO.getStates()) {
					if(statei.equals(statej)) continue;
			...
			bw.write("\n");
			bw.write("; set relation between auxiliary variables");
			bw.write("\n");
			for (Integer statei : mealyIO.getStates()) {
				for (Integer statej : mealyIO.getStates()) {
					if(statei.equals(statej)) continue;
