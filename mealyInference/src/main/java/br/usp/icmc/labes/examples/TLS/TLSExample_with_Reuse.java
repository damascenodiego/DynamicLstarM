...
public class TLSExample_with_Reuse {

	public static void main(String[] args) {
		
		try{

			for (int i = TLSExample_utils.versions.length-1; i > 0; i--) {
				...
				for (int j = 0; j < i; j++) {
					...
					File file_j = new File(TLSExample_utils.versions[j]); 
					File ot_file = new File(new File(out_dir.getParent(),file_j.getName().replaceFirst(".dot$", "")),file_j.getName()+".reuse");
	
					if(ot_file!=null){
						MyObservationTable myot = OTUtils.getInstance().readOT(ot_file,alphabet);
					...
					// learning statistics
					BufferedWriter bw = new BufferedWriter(new FileWriter(new File(out_dir,file.getName()+(ot_file!=null?ot_file.getName():"")+".ot")));
					...
					File sul_ot = new File(out_dir,file.getName()+(ot_file!=null?ot_file.getName():"")+".reuse");
