...
public class RunExperimentRandomWMethod extends RunExperimentAbstract {
	...
	public static void main(String[] args) {
		...
		System.setProperty("logdir", out_dir_string+"/RunExperimentRandomWMethod/");
		System.setProperty("logtstamp", sdf.format(timestamp).replaceAll("/", "_"));

		logger = LearnLogger.getLogger(RunExperimentRandomWMethod.class);
		...
		try {
			...
			if((args.length==1) && args[0].matches("^[0-9]+$")){
				...
				System.err.println("java -cp file.jar uk.ac.le.experiment.RunExperimentRandomWMethod <numberOfRepetitions>");
				System.exit(1);
			}
	...
	protected static void setupEQOracle() {
		...
		eqOracle = new RandomWMethodEQOracle<>(oracleForEQoracle, 2, 20, 20000,rnd_seed,1);
		logger.logEvent("EquivalenceOracle: RandomWMethodEQOracle(2, 20, 20000,"+long_seed+",1)");
