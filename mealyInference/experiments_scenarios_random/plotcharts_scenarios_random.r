...
tab <- tab[grep("^sul.fsm$",tab$SUL),]
tab$Reuse <- gsub("subsul_[0-9]+.fsm.ot$",'REV',tab$Reuse)
...
tab<-cbind(tab,str_split_fixed(tab$Info, " - ", 7))
names(tab)[names(tab)=="1"] <- "Percent"
names(tab)[names(tab)=="2"] <- "Model"
names(tab)[names(tab)=="3"] <- "Num_States"
names(tab)[names(tab)=="4"] <- "Random_id"
names(tab)[names(tab)=="5"] <- "Type"
names(tab)[names(tab)=="6"] <- "Reval_id"
tab$Reuse<-gsub("^N/A NA$","N/A",paste(tab$Reuse,tab$"7"))
tab$"Info"<-NULL; tab$"1"<-NULL; tab$"2"<-NULL; tab$"3"<-NULL; tab$"4"<-NULL; tab$"5"<-NULL; tab$"6"<-NULL; tab$"7"<-NULL;

tab_ok <- tab[grep("^IrfanEQOracle",tab$EqO),]
tab_ok$CorrectType <- paste(gsub("^REV","Dynamic L*M",gsub('^N/A',"L*M",tab_ok$Reuse)),tab_ok$Correct)
tab_ok <- rle(sort(tab_ok$CorrectType))


df_ok <- data.frame(Correct=tab_ok$values, Total=tab_ok$lengths)
df_ok$Method<-"L*M"
df_ok[grep("^Dynamic L\\*M Trunc",df_ok$Correct),]$Method<-"Dynamic L*M (Trunc)"
df_ok[grep("^Dynamic L\\*M Projection",df_ok$Correct),]$Method<-"Dynamic L*M (Projection)"
df_ok$Correct <- gsub("^(Dynamic )?L\\*M ([ProjectionTrunc]+ )?","",df_ok$Correct)
df_ok$Percent <-0
df_ok[df_ok$Method=="Dynamic L*M (Projection)",]$Percent<-df_ok[df_ok$Method=="Dynamic L*M (Projection)",]$Total/sum(df_ok[df_ok$Method=="Dynamic L*M (Projection)",]$Total)
df_ok[df_ok$Method=="Dynamic L*M (Trunc)",]$Percent<-df_ok[df_ok$Method=="Dynamic L*M (Trunc)",]$Total/sum(df_ok[df_ok$Method=="Dynamic L*M (Trunc)",]$Total)
...
for(id_info in c("rmStates","rmInputs","rmInputsStates")){
  for(metric_id in c("L_ms","Rounds","MQ_Resets","MQ_Symbols","EQ_Resets","EQ_Symbols")){
    tab_this<-tab_se[tab_se$"Type"==id_info,]
    
    tab_this$Num_States<-gsub("^0+","",tab_this$Num_States)
    tab_this$Num_States<-as.numeric(tab_this$Num_States)
    
    tab_this <- summarySE(tab_this, measurevar=metric_id, groupvars=c("Num_States", "Cache", "Reuse","CloS","CExH","EqO"))
    title_lab <- paste(metric_id,"@",id_info
                       ,"(IrfanEQOracle)")
    plot <- ggplot(tab_this, aes_string(x="Num_States", y=metric_id,colour="Reuse")) +
      geom_line()+
      ...
      labs(title = title_lab, x = "Number of States")
