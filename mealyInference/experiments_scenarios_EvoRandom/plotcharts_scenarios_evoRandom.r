list.of.packages <- c("ggplot2","reshape2","gtools","stringr","scales","effsize")
...
tab<-cbind(tab,str_split_fixed(tab$Info, " - ", 6))
names(tab)[names(tab)=="1"] <- "sul_path";
names(tab)[names(tab)=="2"] <- "p_id"; tab$p_id<-gsub("^0+","",tab$p_id)
names(tab)[names(tab)=="3"] <- "s_id"; tab$s_id<-gsub("^0+","",tab$s_id)
names(tab)[names(tab)=="4"] <- "Version"   ;
names(tab)[names(tab)=="5"] <- "Reval"; tab$Reval<-gsub("^0+","",tab$Reval)
names(tab)[names(tab)=="6"] <- "ReuseApproach"; tab$ReuseApproach<-gsub("^0+","",tab$ReuseApproach)
...
tab_ok$ReuseApproach<-gsub("^Traditional","L*M",tab_ok$ReuseApproach)
tab_ok$ReuseApproach<-gsub("^First","Dynamic L*M First",tab_ok$ReuseApproach)
tab_ok$ReuseApproach<-gsub("^Previous","Dynamic L*M Previous",tab_ok$ReuseApproach)
tab_ok$CorrectType <- paste(tab_ok$ReuseApproach,tab_ok$Correct)
tab_ok$Method <- tab_ok$ReuseApproach
...
for(metric_id in c("L_ms","Rounds"
  ...
  # print(metric_id)
  tab_this<-(tab_se)
  tab_this$Reuse<-gsub("^[0-9-]+","REV ",tab_this$Reuse)
  tab_this$Reuse<-gsub("^N/A","",tab_this$Reuse)
  tab_this$Reuse<-paste(tab_this$Reuse,tab_this$ReuseApproach,sep = "")
